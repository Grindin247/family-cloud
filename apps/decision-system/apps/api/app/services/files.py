...
def _lexical_score(doc: FileDocument, query_tokens: list[str], query_tags: list[str]) -> tuple[float, list[str]]:
    reasons: list[str] = []
    ...
    name_hits = len(name_tokens.intersection(query_tokens))
    tag_hits = len(tags.intersection({tag.lower() for tag in query_tags}))
    if title_hits:
        score += min(1.0, title_hits / max(1, len(query_tokens))) * 0.35
        reasons.append("Matched title terms")
    if name_hits:
        score += min(1.0, name_hits / max(1, len(query_tokens))) * 0.20
        reasons.append("Matched filename terms")
    if summary_hits:
        score += min(1.0, summary_hits / max(1, len(query_tokens))) * 0.20
        reasons.append("Matched summary terms")
    if body_hits:
        score += min(1.0, body_hits / max(1, len(query_tokens))) * 0.20
    if tag_hits:
...
def _item_type_score(item_type: str) -> float:
    if item_type in {"note", "document"}:
        return 1.0
    if item_type in {"image", "audio", "video"}:
        return 0.7
    if item_type == "folder":
        return 0.35
    return 0.5
...
def _build_embedding_input(payload: FileIndexRequest) -> str:
    parts = [
...
def upsert_file_document(
    ...
    *,
    ...
    existing = db.execute(select(FileDocument).where(FileDocument.path == payload.path)).scalar_one_or_none()
    ...
    if existing is None:
        ...
        existing.media_kind = payload.media_kind
        ...
        existing.is_directory = payload.is_directory
    ...
    db.execute(delete(FileEmbedding).where(FileEmbedding.doc_id == existing.doc_id))
...
def _semantic_scores(
    ...
    *,
    ...
    params: dict[str, object] = {"family_id": family_id, "qvec": vec_literal, "limit": max(top_k * 4, 20)}
    ...
    sql = text(
        ...
        JOIN file_documents d ON d.doc_id = e.doc_id
...
def search_files(
    ...
    *,
    ...
    if payload.content_types:
        query = query.where(FileDocument.content_type.in_(payload.content_types))
    if payload.date_from is not None:
        query = query.where(FileDocument.source_date >= payload.date_from)
    if payload.date_to is not None:
        query = query.where(FileDocument.source_date <= payload.date_to)
    ...
    ranked: list[tuple[float, FileDocument, list[str]]] = []
    ...
    for score, doc, reasons in ranked[: payload.top_k]:
        ...
        content = doc.body_text if payload.include_content and doc.item_type != "folder" else None
        if content:
