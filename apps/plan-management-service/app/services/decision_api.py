...
def ensure_planning_enabled(*, family_id: int, actor_email: str | None, internal_admin: bool) -> None:
    ...
    enabled = next((bool(item.get("enabled")) for item in items if item.get("feature_key") == "planning"), False)
    if not enabled:
        raise_api_error(404, "planning_disabled", "planning domain is disabled for this family", {"family_id": family_id})
...
def list_goals(*, family_id: int, actor_email: str | None, internal_admin: bool) -> list[dict[str, Any]]:
    response = _request(
        "GET",
        "/goals",
        actor_email=actor_email,
    ...
    if response.status_code == 403:
        raise_api_error(403, "family_membership_required", "family membership required", {"family_id": family_id})
    if not response.is_success:
        raise_api_error(502, "decision_api_error", f"decision-system goal listing failed ({response.status_code})")
    payload = response.json()
    items = payload.get("items", []) if isinstance(payload, dict) else None
    if not isinstance(items, list):
        raise_api_error(502, "decision_api_invalid_response", "decision-system goal listing returned invalid items")
    return [item for item in items if isinstance(item, dict)]


def get_goal(*, goal_id: int, actor_email: str | None, internal_admin: bool) -> dict[str, Any]:
    response = _request(
        "GET",
        f"/goals/{goal_id}",
        actor_email=actor_email,
    ...
    if response.status_code == 404:
        raise_api_error(404, "goal_not_found", "goal not found", {"goal_id": goal_id})
    if response.status_code == 403:
        raise_api_error(403, "goal_not_visible", "goal is not visible to this actor", {"goal_id": goal_id})
    if not response.is_success:
        raise_api_error(502, "decision_api_error", f"decision-system goal lookup failed ({response.status_code})")
    payload = response.json()
    if not isinstance(payload, dict):
        raise_api_error(502, "decision_api_invalid_response", "decision-system goal lookup returned invalid JSON")
    return payload
