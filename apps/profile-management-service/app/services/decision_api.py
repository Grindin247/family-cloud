...
def ensure_profile_enabled(*, family_id: int, actor_email: str | None, internal_admin: bool) -> None:
    ...
    enabled = next((bool(item.get("enabled")) for item in items if item.get("feature_key") == "profile"), False)
    if not enabled:
        raise_api_error(404, "profile_disabled", "profile domain is disabled for this family", {"family_id": family_id})
